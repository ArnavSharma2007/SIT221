...
namespace Vector
{
    public class Vector<T>
    {
        ...
        {
            //To check whether the index could be present in array or not
            ...
            //To check if the array still has 1 more value left ONLY
            if (index == Count)
            {
                Add(item);
                return;
            ...
            //Shifting the elements to the left to allow addition of the element at index
            for (int i = Count; i > index; i--)
            {
                data[i] = data[i - 1];
            }

            //Add value at index and increase count
            data[index] = item;
            Count++;
        }
        ...
        public void Clear()
        {
            //Set count to 0
            Count = 0;
            //Create a new array to replace the old one
            data = new T[Capacity];
        }

        public bool Contains(T item)
        {
            //Calling the pre-existing function
            return IndexOf(item) != -1;
        }

        public bool Remove(T item)
        {
            //Figure out the index of the item to reduce the number of functions created
            int index = IndexOf(item);

            //Check whether item exists
            if (index != -1)
            {
                //Call RemoveAt to, again, reduce the number of functions
                RemoveAt(index);
                return true;
            }
        ...
        public void RemoveAt(int index)
        {
            ...
            //Use a loop to overwrite the value at index, and set the other values over it
            for (int i = index; i < Count - 1; i++)
            {
                data[i] = data[i + 1];
            }

            //Decrement count as 1 value was reduced
            Count--;
        }

        public override string ToString()
        {
            //Check whether array has no elements in it
            if (Count == 0)
            {
                return "[]";
            }

            //Using StringBuilder as said in tasksheet 
            StringBuilder sb = new StringBuilder();
            sb.Append("[");

            //Run a loop to go through each element one by one
